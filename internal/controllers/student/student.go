...
import (
	...
	"crud_app_thedynamodbbook/internal/entities/student"
...
type Interface interface {
	ListOne(ID uuid.UUID) (entity student.Student, err error)
	ListAll() (entities []student.Student, err error)
	Create(entity *student.Student) (uuid.UUID, error)
	Update(ID uuid.UUID, entity *student.Student) error
...
func (c *Controller) ListOne(id uuid.UUID) (entity student.Student, err error) {
	entity.ID = id
	response, err := c.repository.FindOne(entity.GetFilterId(), entity.TableName())
	if err != nil {
		return entity, err
	}
	return student.ParseDynamoAtributeToStruct(response.Item)
}

func (c *Controller) ListAll() (entities []student.Student, err error) {
	...
	if response != nil {
		for _, value := range response.Items {
			entity, err := student.ParseDynamoAtributeToStruct(value)
			if err != nil {
				return entities, err
			}
...
func (c *Controller) Create(entity *student.Student) (uuid.UUID, error) {
	entity.CreatedAt = time.Now()
	_, err := c.repository.CreateOrUpdate(entity.GetMap(), entity.TableName())
	return entity.ID, err
}

func (c *Controller) Update(id uuid.UUID, entity *student.Student) error {
