...
import (
	...
	"crud_app_thedynamodbbook/internal/controllers/student"
	EntityStudent "crud_app_thedynamodbbook/internal/entities/student"
	"crud_app_thedynamodbbook/internal/handlers"
	"crud_app_thedynamodbbook/internal/repository/adapter"
	Rules "crud_app_thedynamodbbook/internal/rules"
...
type Handler struct {
	...
	Controller student.Interface
	Rules      Rules.Interface
}

func NewHandler(repository adapter.Interface) handlers.Interface {
	return &Handler{
		Controller: student.NewController(repository),
		Rules:      RulesStudent.NewRules(),
	}
}
...
func (h *Handler) getBodyAndValidate(r *http.Request, ID uuid.UUID) (*EntityStudent.Student, error) {
	productBody := &EntityStudent.Student{}
	body, err := h.Rules.ConvertIoReaderToStruct(r.Body, productBody)
	if err != nil {
		return &EntityStudent.Student{}, errors.New("body is required")
	}

	productParsed, err := EntityStudent.InterfaceToModel(body)
	if err != nil {
		return &EntityStudent.Student{}, errors.New("error on convert body to model")
	}
...
func setDefaultValues(product *EntityStudent.Student, ID uuid.UUID) {
